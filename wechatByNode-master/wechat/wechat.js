...
WeChat.prototype.getTicket = function(req,res){
  var that = this;
  var su=req.query.url;
  console.log(su);
  this.getAccessToken().then(function(data){
    //  console.log(data);
    //获取当前时间
    var currentTime = new Date().getTime();
    //格式化请求连接
    var url = util.format(that.apiURL.ticket,that.apiDomain,data);
     //判断 本地存储的 ticket 是否有效
     if(accessTokenJson.ticket === "" || accessTokenJson.ticket_time < currentTime){
         that.requestGet(url).then(function(data2){
             var result = JSON.parse(data2);

             if(data2.indexOf("errcode") < 0){
                 accessTokenJson.ticket = result.ticket;
                 accessTokenJson.ticket_time = new Date().getTime() + (parseInt(result.expires_in) - 200) * 1000;
                 //更新本地存储的
                 fs.writeFile('./wechat/access_token.json',JSON.stringify(accessTokenJson));
                 //将获取后的 access_token 返回
                 var Ticket = sign(result.ticket, su)
                 //console.log(sign(JSON.parse(data2).ticket, 'http://119.3.19.7/static/index.html'));
                 res.send(Ticket);
             }else{
                 //将错误返回
                 //res.send(result);
         ...
         //将本地存储的 access_token 返回
         //res.send(accessTokenJson.ticket);
          console.log(11);
         var Ticket = sign(accessTokenJson.ticket, su)
         //console.log(sign(JSON.parse(data2).ticket, 'http://119.3.19.7/static/index.html'));
         res.send(Ticket);
     }
